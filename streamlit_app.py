...
QUICK_PROFILES_NEW = {
    ...
    "🇮🇱 ישראל": {"foreign_max": 10},
    '💱 מט"ח':   {"fx_min": 90},
    "🔵 כללי":   {"track_contains": "כללי"},  # include all general tracks whose name contains "כללי"
}
...
<div style='font-size:13px;font-weight:800;color:#1F3A5F;margin-bottom:8px'>
  📊 {_esc(selected_alt)} — פרטים
            ...
            # Voting stats – use session_state flag so it survives rerun
            if voting_configured:
  ...
  setTimeout(function(){
    ...
    with st.expander("⚖️ השוואת מסלולי השקעה", expanded=auto_open):
        ...
        if qp and qp in QUICK_PROFILES_NEW:
            ...
            if "track_exact" in pf: filt = filt[filt["track"].astype(str).str.strip() == pf["track_exact"]]
            if "track_contains" in pf: filt = filt[filt["track"].astype(str).str.contains(pf["track_contains"], na=False)]
